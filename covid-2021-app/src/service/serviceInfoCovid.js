...
export default class serviceInfoCovid {
  ...
  getInfoDepartement2() {
    return new Promise(function(resolve, reject) {
      axios
        .get(
          "https://coronavirusapi-france.now.sh/AllDataByDate?date=" + this.date
        )
        .then(response => {
          resolve(response);
        })
  ...
  async getInfoRegion() {
    ...
    /*
    let result = await this.getInfoDepartement2();
    let copieResult = JSON.parse(
      JSON.stringify(result.data.allFranceDataByDate)
    );

    let AuvergneRhoneAlpesSum = AuvergneRhoneAlpes.reduce((a, e) => {
      return a.concat(copieResult.filter(c => c.code === e));
    }, []).reduce((a, e) => a + e.nouvellesHospitalisations, 0);
  ...
  getGlobalDataFranceByDate(today) {
    let url =
      "https://coronavirusapi-france.now.sh/FranceGlobalDataByDate?date=" +
      today;
    console.log(
      "https://coronavirusapi-france.now.sh/FranceGlobalDataByDate?date=" +
        today
  ...
  getCasCovidFranceFromTo(date1, date2) {
    let url =
      "https://api.covid19api.com/total/country/france/status/confirmed?from=" +
      date1 +
      "T00:00:00Z&to=" +
      date2 +
      "T00:00:00Z";
  ...
  async getResultCountry(destination) {
    let url =
      "https://coronavirus-19-api.herokuapp.com/countries/" + destination;
    return new Promise(function(resolve, reject) {
      axios
        .get(url)
        .then(response => {
